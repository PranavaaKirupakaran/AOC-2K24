...
int main(){
    ...
    while(1){
        ...
        if(num_char_read != 1){
            ...
            if(input_char == 'm' && command_index == 0){
                command_index++;
            }
            else if(input_char == 'u' && command_index == 1){
                command_index++;
            }
            else if(input_char == 'l' && command_index == 2){
                command_index++;
            }
            else if(input_char == '(' && command_index == 3){
                command_index++;
            }
            else if(input_char >= '0' && input_char <= '9' && command_index >= 4 && !(first_number_done)){
                *(first_number + first_number_offset) = input_char - '0';
                first_number_offset++;
                command_index++;
            }
            else if(input_char == ',' && command_index == 4 + first_number_offset){
                command_index++;
                first_number_done = true;
            }
            else if(input_char >= '0' && input_char <= '9' && command_index >= 5 + first_number_offset){
                *(second_number + second_number_offset) = input_char - '0';
                second_number_offset++;
                command_index++;
            }
            else if(input_char == ')' && command_index == 5 + first_number_offset + second_number_offset){
